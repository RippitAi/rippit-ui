    ...
    <div className="flex h-full flex-col">
      ...
      <div className="relative flex min-h-0 flex-1 items-center justify-center bg-vpbg p-6" style={{ backgroundImage: "radial-gradient(var(--dot) 1.2px, transparent 1.6px)", backgroundSize: "24px 24px" }}>
        <div className="w-full max-w-[440px] rounded-card border border-line bg-panel p-5 shadow-[var(--shadow-card)] backdrop-blur-[14px] anim-fade-up">
          <h1 className="text-[14px] font-semibold tracking-[-0.01em]">Pick a workflow</h1>
          ...
          {!railOpen && (
            <button
              ...
              className="mt-3 inline-flex h-8 cursor-pointer items-center gap-2 rounded-control border border-line-strong px-3 text-[12px] font-semibold text-t1 transition-colors hover:bg-hover"
            >
          ...
          {recent.length > 0 && (
            <section className="mt-4" aria-label="Recently opened">
              <p className="mb-1 text-[9.5px] font-semibold uppercase tracking-wide text-t3">Recent</p>
              <div className="flex flex-col">
                {recent.map((r) => (
                  <Link key={`${r.provider}:${r.id}`} href={`/w/${r.provider}/${r.id}`} className="flex h-8 items-center gap-2 rounded-row px-2 text-[12px] text-t2 transition-colors hover:bg-hover hover:text-t1">
          ...
          {suggestions.length > 0 && (
            <section className="mt-4" aria-label="Workflows">
              <p className="mb-1 text-[9.5px] font-semibold uppercase tracking-wide text-t3">Workflows</p>
              <div className="flex flex-col">
                {suggestions.map((w) => (
                  <Link key={`${w.provider}:${w.refId}`} href={workflowHref({ source: w.provider, refId: w.refId })} className="flex h-8 items-center gap-2 rounded-row px-2 text-[12px] text-t2 transition-colors hover:bg-hover hover:text-t1">
          ...
          {!loading && connections.length === 0 && (
            <p className="mt-4 text-[12px] text-t2">
